---
name: mobile-design
description: Diseño responsive mobile-first del Dashboard Maestro en React + MUI v6. El panel debe funcionar correctamente en celular (el superadmin lo usará desde el teléfono). Usar para decisiones de layout móvil, touch targets, sidebar colapsable, tablas responsive, y breakpoints MUI.
allowed-tools: Read, Glob, Grep, Bash
---

## Mobile-First Dashboard Maestro

### Breakpoints MUI a usar
```javascript
...
### Sidebar responsive
- **xs/sm**: drawer temporal (se cierra al navegar)
- **md+**: sidebar permanente (240px expandido, 64px colapsado)
- Toggle con `MenuRoundedIcon` en AppBar

### Tablas en mobile
```jsx
// Siempre envolver con overflow
<TableContainer sx={{ overflowX: 'auto' }}>
  <Table sx={{ minWidth: 500 }}>
```
Considerar ocultar columnas secundarias en xs:
```jsx
<TableCell sx={{ display: { xs: 'none', sm: 'table-cell' } }}>
```

### Touch targets
- Botones mínimo 44×44px (MUI lo cumple por defecto con `size="medium"`)
- `IconButton` con `size="large"` en mobile para facilitar toque
- Espaciado entre elementos interactivos: mínimo 8px

### Cards de métricas en mobile
```jsx
// xs=12 para que sean full width en mobile, sm=6 en tablet, md=3 en desktop
<Grid item xs={12} sm={6} md={3}>
```

### Login page en mobile
- Card centrada con `maxWidth: 400px` y `mx: 'auto'`
- `px: 2` en mobile para no pegar a los bordes
- Inputs con `fullWidth`

### Consideraciones de performance mobile
- Lazy loading de páginas con React.lazy + Suspense
- No cargar la tabla de tenants hasta que el usuario haga login
- Skeleton loading en lugar de spinners (mejor UX percibida)
